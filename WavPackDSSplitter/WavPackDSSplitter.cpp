...
AMOVIESETUP_MEDIATYPE sudInput1Type[] =
{
...
AMOVIESETUP_PIN sudPins[] =
{
    { L"Input",
        ...
        NULL,                           // ConnectsToPin
        NUMELMS(sudInput1Type),          // Number of media types
        sudInput1Type
    },
    { L"Output",
        FALSE,                          // bRendered
...
AMOVIESETUP_FILTER sudWavpackSplitter =
{
    &CLSID_WavPackDSSplitter,
    g_wszWavPackDSName,
    MERIT_NORMAL,
    NUMELMS(sudPins),
    sudPins
...
{
  {
    g_wszWavPackDSName,
    &CLSID_WavPackDSSplitter,
    CWavPackDSSplitter::CreateInstance,
    NULL,
    &sudWavpackSplitter
  }
};
...
{
	RegisterSourceFilterPattern("0,4,,7776706B", CLSID_AsyncReader,
		MEDIATYPE_Stream, MEDIASUBTYPE_WAVPACK_Stream);

    RegisterSourceFilterExtension(".wv", CLSID_AsyncReader,
		MEDIATYPE_Stream, MEDIASUBTYPE_WAVPACK_Stream);
...
{
    DebugLog("CWavPackDSSplitter::CWavPackDSSplitter... 0x%08X", GetCurrentThreadId());
...
{    
    ...
    DebugLog("CWavPackDSSplitter::BeginFlush... 0x%08X", GetCurrentThreadId());
...
HRESULT CWavPackDSSplitter::EndFlush()
{
    ...
    DebugLog("CWavPackDSSplitter::EndFlush... 0x%08X", GetCurrentThreadId());
...
{
    DebugLog("CWavPackDSSplitter::DoSeeking... 0x%08X", GetCurrentThreadId());
...
{
    DebugLog("CWavPackDSSplitterInputPin::CheckMediaType 0x%08X", GetCurrentThreadId());
...
{
    DebugLog("CWavPackDSSplitterInputPin::CheckConnect 0x%08X", GetCurrentThreadId());
    ...
    if (FAILED(hr))
    {
        DebugLog("CWavPackDSSplitterInputPin::CheckConnect QueryInterface failed 0x%08X", GetCurrentThreadId());
        return S_FALSE;
    }
...
HRESULT CWavPackDSSplitterOutputPin::GetMediaType(int iPosition, CMediaType *pMediaType)
{
    ...
    if (wpp->channel_count == 2 && wpp->several_blocks)
        wcpd.flags |= WPFLAGS_SEVERALBLKS;
...
{
    DebugLog("CWavPackDSSplitterCorrectionInputPin::CheckMediaType 0x%08X", GetCurrentThreadId());
...
{
    DebugLog("CWavPackDSSplitterCorrectionInputPin::CheckConnect 0x%08X", GetCurrentThreadId());
...
{
    DebugLog("CWavPackDSSplitterCorrectionInputPin::BreakConnect 0x%08X", GetCurrentThreadId());
