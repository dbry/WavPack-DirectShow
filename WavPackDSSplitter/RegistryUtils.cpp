...
char* GetToken(char* src, const char* sep, int& position) {
    char* res = src + position;
...
int ContainsExt(const char* Src, const char* Extension) {
    if (Src == NULL) {
        return 0;
    }
    char* SrcDup = _strdup(Src);
    char* token = NULL;
    int position = 0;
    while (position != -1) {
        char* token = GetToken(SrcDup, ";", position);
